...
import { Menu, Fish, Boxes, ShoppingCart, Users, Home, Landmark, Banknote, Wallet, UserCircle, Building2, FileText, Package, DollarSign, Calculator, Tag, UsersRound, BarChart3, ChevronDown, LogOut } from "lucide-react";
    ...
    >
      <div className={`flex h-full w-64 flex-col gap-2 overflow-y-auto p-4 ${isOpen ? "opacity-100" : "opacity-0"}`}>
        ...
        <div className="mt-auto border-t pt-4">
          <Button
            variant="ghost"
            className="w-full justify-start gap-2 text-[var(--sidebar-foreground)]"
        ...
        <div className="text-xs text-[var(--muted-foreground)]">© {new Date().getFullYear()} FinWave</div>
      </div>
    </aside>
  );
}
